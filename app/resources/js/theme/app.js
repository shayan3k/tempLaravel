...
$("body").on('keyup', 'input.divide-price', function(a) {
...
$("body").on('keydown', 'input.digits', function(a) {
    if ($.inArray(a.keyCode, [46, 8, 9, 27, 13, 190]) !== -1 || a.keyCode == 65 && a.ctrlKey === true || a.keyCode >= 35 && a.keyCode <= 39) {
        return
    }
...
/*
* Blocks the screen with blockUI plugin
*/
function blockScreen(loading_wrapper)
{
    loading_wrapper = loading_wrapper || '#state-loading';
    $.blockUI({
        message: $(loading_wrapper),
        css: {
...
/*
* Shows a hidden the Selectbox and it's wrapper and put the data in select box
*/
function showInput(wrapper, input, data)
{
    $(wrapper).slideDown();
    $(input).html(data).trigger("liszt:updated");
}

/*
* Removes a Selectbox and it's wrapper
*/
function removeInput(wrapper, input)
{
    input = input || '';
    if(input != ''){
        $(input).html("<option value='any'>همه</option>").trigger("liszt:updated");
    }
...
/*
* Shows an errors toast with given text
*/
...
/**
 * Starts a new selectpicker
 *
 */
function startSelectPicker(long, lat, zoom, selector) {
    selector = selector || $("#map-convas");
    zoom = zoom || 12
    ...
    selector.locationpicker({
        ...
        zoom: zoom,
        inputBinding: {
            latitudeInput: $("#frm_latitude"),
            longitudeInput: $("#frm_longitude"),
            locationNameInput: $("#frm_locationName")
        },
